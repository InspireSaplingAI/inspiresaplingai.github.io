...
export const GET: APIRoute = async ({ request, locals }) => {
    ...
    // Page-based pagination: each page returns ~10 jobs (num_pages=1).
    ...
    // The frontend combines the job title + location into a single query
    // string (e.g. "Software Engineer in New York"), so we pass it through
    // as-is. `country=us` keeps results US-focused.
    ...
    // JSearch returns ~10 jobs per page. If we got a full page, there's
    // likely a next page — tell the frontend to show "View More".
    const hasMore = mapped.length >= 10
